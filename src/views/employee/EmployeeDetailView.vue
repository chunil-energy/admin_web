...
export default {
  ...
  async setup() {
    ...
    const onSubmit = handleSubmit(async (fv) => {
      ...
      console.log('submit ', fv)
      const response = await createEmployee(userId, fv)
      console.log(response)
      // const response = await createEmployeeUser(fv)
      // if (response.success) {
      ...
      //   await errorStore.set('error', '사용자 등록 실패', response.message)
      // }
    })
    return {
      errorStore,
  ...
  <div class="px-4 sm:px-6 lg:px-8">
    <div class="sm:flex sm:items-center">
      <div class="sm:flex-auto">
        <h1 class="text-base font-semibold leading-6 text-gray-900">임직원 상세정보</h1>
      </div>
    </div>
          ...
          <div class="border-b border-gray-900/10 pb-12">
            <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-9">
              <div class="col-span-1">
                ...
                <div class="mt-2">
                  <input type="text" id="username" disabled readonly :value="userData?.user.username"
