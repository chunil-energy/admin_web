            ...
            <div class="flex items-start">
              <div class="flex-shrink-0">
                ...
                <template v-else-if="error.status === 'warning'">
                  <MinusCircleIcon class="h-6 w-6 text-yellow-400" aria-hidden="true"/>
                </template>
              </div>
              <div class="ml-3 w-0 flex-1 pt-0.5">
                <template v-if="error.status === 'error'">
                  <p class="text-sm font-medium text-red-400">{{ error.title }}</p>
                </template>
                <template v-else-if="error.status === 'success'">
                  <p class="text-sm font-medium text-gray-400">{{ error.title }}</p>
                </template>
                <template v-else-if="error.status === 'warning'">
                  <p class="text-sm font-medium text-yellow-400">{{ error.title }}</p>
                </template>
