export const generateRandomColors = (numColor) => {
    /*
    ...
    */
    ...
    var loadedColors = typeof (arguments[1]) == 'undefined' ? [] : arguments[1],//predefine colors in the set
        numColor = numColor + loadedColors.length,//reset number to include the colors already passed
        ...
        rgbToHSL = function (rgb) {//converts [r,g,b] into [h,s,l]
            ...
            var breakdownAt = 64, //if you're generating this many colors or more, don't try so hard to make unique hues, because you might fail.
                ...
                breakVal = loadedColors.length / numColor * (numColor - breakdownAt), //break down progressively (if it's the second color, you can still make it a unique hue)
    ...
    for (var i = loadedColors.length; i < numColor; i++) { //Start with our predefined colors or 0, and generate the correct number of colors.
